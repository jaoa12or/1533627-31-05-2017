...
int main() {
	...
	roomempty = sem_open("mysem15", O_CREAT,0600,1);
	mutex = sem_open("mysem16", O_CREAT,0600,1);
...
void  INThandler(int sig)
{
     ...
     c = getchar();
     if (c == 'y' || c == 'Y') {
 		sem_close(roomempty); sem_unlink("mysem12");
 		sem_close(mutex); sem_unlink("mysem13");
		exit(0);
     } else
          	signal(SIGINT, INThandler);
